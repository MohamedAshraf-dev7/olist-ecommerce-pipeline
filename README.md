...
Bronze (data/)  --clean_*.py-->  Silver (silver/)  --build_gold.py-->  Gold (gold/)
   raw CSVs                    cleaned, validated                  dim_customers.csv
                                 1:1 with source                    fact_orders.csv
```
...
- **Gold** — a star schema built from the cleaned silver tables:
  one dimension table (`dim_customers`) and one fact table
...
├── gold/                  # dim_customers.csv, fact_orders.csv
...
│   └── clean_reviews.py
├── build_gold.py          # silver -> gold (dim + fact tables)
...
Product- and seller-level detail was deliberately left out of the fact
table: an order can contain multiple products/sellers, so a proper
product dimension would require a second, item-grain fact table
(`fact_order_items`) rather than forcing a many-to-one relationship
into the order-grain table. Out of scope for this pass.
...
| products | Rows with 0 dimensions | Dropped (physically impossible) |
...
`test_cleaning.py` runs 52 automated checks across every table in both
